...
func TestClusterState_AssumeAndReserve(t *testing.T) {
	tests := []struct {
		...
		{
			name:                 "pod without node name in pod.Spec.NodeName but with node name passed to AssumeAndReserve",
			nodeNames:            []string{"node1"},
			podNodeName:          "node1",
		...
		{
			name:    "nil nodeName returns error",
			pod:     st.MakePod().Name("pod1").Namespace("default").UID("uid-pod1").Node("node1").Obj(),
			wantErr: true,
		},
		{
			name:        "unassume pod where nodeName != pod.Spec.NodeName returns error",
			pod:         st.MakePod().Name("pod1").Namespace("default").UID("uid-pod1").Node("node1").Obj(),
			podNodeName: "node2",
			wantErr:     true,
		},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !tc.nilCache {
				internalCache = cache.New(ctx, nil, false, false)
			...
			if tc.duplicateAssume {
				if err := state.AssumeAndReserve(ctx, tc.pod, cycleState, tc.podNodeName); err != nil {
					t.Fatalf("First AssumeAndReserve() unexpected error = %v", err)
				}
			}

			err = state.AssumeAndReserve(ctx, tc.pod, cycleState, tc.podNodeName)
			if (err != nil) != tc.wantErr {
