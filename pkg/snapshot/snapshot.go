// Package snapshot provides types and implementations for cluster snapshots.
package snapshot
...
func (s *clusterSnapshot) SchedulePods(ctx context.Context, pods []SchedulablePod, opts SchedulingOptions) ([]SchedulingResult, error) {
	...
	for _, pod := range pods {
		...
		if !success {
			if opts.StopOnFailure {
				return result, fmt.Errorf("pod %s/%s could not be scheduled on any candidate node", pod.Pod.Namespace, pod.Pod.Name)
			}
		}
	}
...
func (s *clusterSnapshot) PreemptPods(ctx context.Context, pods []*v1.Pod) (PreemptionSnapshot, error) {
	...
	for _, pod := range pods {
		...
		if nodeName == "" {
			return newPreemptionSnapshot(s, preemptedPods, nodeNames), fmt.Errorf("pod %s/%s has no node name", pod.Namespace, pod.Name)
		}
		...
		if err != nil {
			return newPreemptionSnapshot(s, preemptedPods, nodeNames), fmt.Errorf("failed to unreserve and forget pod %s/%s: %v", pod.Namespace, pod.Name, err)
		}
...
func (ps *preemptionSnapshot) Unpreempt(ctx context.Context) error {
	...
	for i, pod := range ps.pods {
		...
		if err := ps.snapshot.assumeAndReserve(ctx, pod, nodeName, cycleState); err != nil {
			return fmt.Errorf("failed to assume and reserve pod %s/%s during unpreempt: %v", pod.Namespace, pod.Name, err)
		}
	}
...
func (c *clusterSnapshot) assumeAndReserve(ctx context.Context, pod *v1.Pod, nodeName string, cycleState *schedulerframework.CycleState) error {
	...
	c.schedulerSnapshot.AssumePod(podInfo)
